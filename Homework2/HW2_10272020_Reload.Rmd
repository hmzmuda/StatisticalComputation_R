...
I <- function(N, b1, b2, theta.old){
  result11 <- sum(N*(sum((N*b1^2)/(theta.old)^2)))#1st row, 1st col
  result12 <- sum(N*(sum((N*b1*b2)/(theta.old)^2)))#non-principal components
  result22 <- sum(N*(sum((N*b2^2)/(theta.old)^2)))#2nd row, 2nd col
  output <- as.numeric(list(result11,result12,result12,result22))
...
new.oil <- function(N,b1,b2,dl,d2l){
  n <- length(N)
  i <- 1
  theta.old <- c(-2,-2)#need it to be a 2x26 matrix, but not sure what inital value to assign to it.
  ...
  for(i in 1:n){
    ...
    # if(abs(c(theta.new[1],theta.new[2])-theta.old) > 1e-6){
    #   theta.old <- c(theta.new[1],theta.new[2])
    # }
    # else{
    #   theta.values[i+1,] <- theta.new
    # }
...
fish.oil <- function(N,b1,b2,dl,I){
  ...
  for(i in 1:n){
    ...
    if(abs(c(theta.new[1],theta.new[2]) - theta.old) <= c(1e-6,1e-6)){
      theta.values[i+1,] = theta.new
   }
   theta.old <- c(theta.new[1],theta.new[2])
  }
...
We are given a Weibull distribution with the density function $f(x) = abx^{b-1}exp(-ax^b)$ for $0 < x$ and for the parameters $a$ and $b$.
\[ 
  \begin{cases} 
...
This can be found by $l(a,b|X_i, \delta_i) = \Sigma_{i = 1}^{n}log(f(X_i,\delta_i | a,b))$. Expanding this equation out we get:
...
$$Q(a,b|a^{(t)},b^{(t)}) = E[l(a,b|X_i, \delta_i)]$$
$$Q(a,b|a^{(t)},b^{(t)}) = \Sigma_{i = 1}^{n} \delta_i (log(a) + log(b)) - E[\Sigma_{i = 1}^{n}ax_i^b] + E[log(\Sigma_{i = 1}^{n}x_i^{\delta_i(b-1)})]$$
$$Q(a,b|a^{(t)},b^{(t)}) = \Sigma_{i = 1}^{n} \delta_i (log(a) + log(b)) - \Sigma_{i = 1}^{n}x_i(\Sigma_{i = 1}^{n}ax_i^b) + \Sigma_{i = 1}^{n}x_i(log(\Sigma_{i = 1}^{n}x_i^{\delta_i(b-1)}))$$
As we can see from this equation, we have $x_i$ in multiple parts of the Q function. Because of this, the Q function is difficult to compute analytically. One work around for this is to calculate an approximation of the $Q$ function, $\hat{Q}$.
$$\hat{Q}(a,b|a^{(t)},b^{(t)}) = \frac{1}{m^{(t)}}\Sigma_{j = 1}^{m^{(t)}}L(a,b | X_j)$$
