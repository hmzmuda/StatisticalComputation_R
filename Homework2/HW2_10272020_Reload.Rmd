...
print(optim.error)
```
Based on the output from the three methods, i would conclude that utilizing the "optim" function would be the easiest and more accurate way to implement MLE. It requires less coding (which equals less error) and produces a reasonable standard error. I would put it just in front of Newton's method because while the "optim" method is easier, the Newton's method is able to produce a comparable standard error with fewer iterations. I would put the Fisher Information approach last because it is computationally intensive with calculating the expectation and having to calculate the first and second derivative of the log likelihood. Moreover, it produced the worst standard error and required more iterations to find the MLE within the tolerance.
...
Because there are multiple local maxima in the equation, we cannot use this method for the EM algorithm for this distribution.
