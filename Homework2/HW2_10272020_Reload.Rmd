...
I <- function(N, b1, b2, theta.old){
  result11 <- sum((b1^2)/(theta.old[1]*b1 + theta.old[2]*b2)^2)#1st row, 1st col
  result12 <- sum((b1*b2)/(theta.old[1]*b1 + theta.old[2]*b2)^2)#non-principal components
  result22 <- sum((b2^2)/(theta.old[1]*b1 + theta.old[2]*b2)^2)#2nd row, 2nd col
  output <- as.numeric(list(result11,result12,result12,result22))
...
new.oil <- function(N,b1,b2){
  ...
  for(i in 1:n){
    ...
    if(abs(theta.new[1] - theta.old[1]) <= 1e-6){
      ...
      return(list("list" = theta.values, root = tail(theta.values,n = 1)))
      }
    }
  }
  return(list("list" = theta.values, root = tail(theta.values,n = 1)))
...
#     if(abs(theta.new[1] - theta.old[1]) <= 1e-6){
#       if(abs(theta.new[2] - theta.old[2]) <= 1e-6){
#         output = list("list" = theta.values, root = tail(theta.values,n = 1))
#       return(output)
#       }
#     }
#   }
...
fish.oil <- function(N,b1,b2){
  ...
  tolerance <- 100
  toll <- 10^(-10)
  while(tolerance > toll){
    theta.new <- theta.old + (solve(I(N,b1,b2,theta.old)) %*% dl(N,b1,b2,theta.old))
    #theta.values[i+1,] <- theta.new
    root <- theta.new
    theta.old <- theta.new
    i <- i + 1
    tolerance <- sum(abs(theta.new-theta.old))
  }
  return(root)
}
...
newSE = sqrt(solve(I(N,b1,b2,output.new$root)))
fishSE = sqrt(solve(I(N,b1,b2,output.fish$root)))
```
