...
$$= \Sigma_{i = 1}^{n} E[\delta_i (log(a) + log(b)) - ax_i^b + (b-1)log(x_i) | y,\theta^{(t)}]$$
$$= \Sigma_{i = 1}^{n} \delta_i (log(a) + log(b)) - \Sigma_{i = 1}^{n}x_i(\Sigma_{i = 1}^{n}ax_i^b) + \Sigma_{i = 1}^{n}x_i(log(\Sigma_{i = 1}^{n}x_i^{\delta_i(b-1)}))$$
