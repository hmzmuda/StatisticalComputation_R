...
data <- runif(n,0,theta) #uniform distribution
T.boot <- rep(0,itr)
T.star.boot <- rep(0,itr)
T.f <- function(x){-n*(x-theta)}
T.star.f <- function(x,x.star){-n*(x.star-x)}
#x.star is pseudo bootstrap data

for(i in 1:itr){
  x.star = sample(data,n,replace = TRUE)
  x <- data
  T.boot[itr] <- T.f(x)
  T.star.boot <- T.star.f(x,x.star)
}
