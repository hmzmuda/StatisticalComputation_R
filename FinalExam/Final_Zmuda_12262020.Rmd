...
alpha[1] = rnorm(1,0,sigma)
beta[1] = rnorm(1,0,sigma)
mu[1] = rnorm(1,0,sigma)
lambda[1] = rnorm(1,0,sigma)
...
delta.h <- c(rep(0,length(hormone.rec)),rep(1,length(hormone.cens)))
delta.c <- c(rep(0,length(control.rec)),rep(1,length(control.cens)))
...
#FUNCTIONS
prior <- function(theta,tau){
  return((theta^a)*(tau^b)*exp(-c*theta - d*theta*tau))
}
posterior <- function(dat.control,dat.hormone,theta,tau){
  one <- theta^(sum(dat.control$delta.c)+sum(dat.hormone$delta.h)+a)
  two <- tau^(sum(dat.hormone$delta.h)+b)
  three <- exp(-theta*sum(dat.control$x)-tau*theta*sum(dat.hormone$x))
  four <- exp(-(c*theta)-(tau*d))
...
colMeans(gibby)
gibbies <- gibbs(dat.control,dat.hormone,5000,1000,c(3,1,60,120))
colMeans(gibbies)
...
hg.prior <- hist(prior(gibbies[,1],gibbies[,2]),breaks = 40,plot = FALSE)
hg.posterior <- hist(posterior(dat.control,dat.hormone,gibbies[,1],gibbies[,2]),breaks = 40,plot = FALSE)
plot(hg.prior, col = c1)
plot(hg.posterior, col = c2,add = TRUE)
...
Based on the summary statistics produced in part d, I would say that the recurrence times are not statistically significant from the control group. This would mean that the clinical trial was not a huge success because the drug has very little affect on combating recurring episodes of breast cancer.
...
alpha <- 0.05
hormone.boot <- sort(sample(hormone.estimate, size = B, replace = TRUE))
control.boot <- sort(sample(control.estimate, size = B, replace = TRUE))
j <- (alpha/2) * B
k <- (1-(alpha/2)) * B
cat("95% CI for Hormone Group:", c(hormone.boot[j],hormone.boot[k]),"\n")
cat("95% CI for Control Group:", c(control.boot[j],control.boot[k]),"\n")
...
R <- 1000 # number of permutation replicates
x <- sort(hormone.estimate)
y <- sort(control.estimate)
z <- c(x,y)
D <- numeric(R)
...
options(warn = -1)
for(i in 1:R){
  k <- sample(K,size = length(hormone.rec),replace = FALSE)
  x1 <- z[k] #hormone group
  y1 <- z[-k] #control group
