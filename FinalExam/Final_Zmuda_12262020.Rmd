...
$$Q(\theta | \theta^{(t)}) = \Sigma^{16}_{i = 0}[ \frac{n_0z_{0}log(\alpha 1_{i = 0})}{N} +$$
$$
\frac{n_i(t_i)[log(\beta \mu^i e^{-\mu}) + log((1-\alpha-\beta)\lambda^ie^{-\lambda})]}{N} +$$
$$\frac{n_i(p_i)[log(\beta \mu^i e^{-\mu}) + log((1-\alpha-\beta)\lambda^ie^{-\lambda})]}{N} - log(i!)]$$
**M step:**
...
#hist(data$freq, breaks = 17)
#vline(pois(data$freq))
...
\newpage
# Problem 2
## part a: Metropolish-Hastings Algorithm
...
param = c(alpha,beta,mu,lambda)
U <- runif(N)
#use a normal distribution as a starting distribution with mean 0 and sd 1
start.dist <- function(x){
  if(x<0){return(0)}
  else{return((1/sqrt(2))*exp(0.5*(x)^2))}
}
#Our target distribution is a poisson distribution
target.dist <- function(x,lambda){
  if(x<0){return(0)}
  else{return((lambda^x*exp(x))/factorial(x))}
}
...
for(i in 2:N){
  xt <- x[i-1]
  y <- rnorm(1,0,1) + xt
  num <- target.dist(y,lambda)*start.dist(xt)
  dem <- target.dist(xt, lambda)*start.dist(y)
  
  if(U[i] <= num/dem){x[i] <- y}
  else{
    x[i] <- xt
    reject = reject + 1
  }
