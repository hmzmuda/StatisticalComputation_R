...
$$log[L(\theta | n_{k,i})] = \Sigma^{16}_{i = 0} n_ilog(\alpha 1_{i = 0} + \beta \mu^i e^{-\mu} + (1-\alpha-\beta)\lambda^ie^{-\lambda}))$$
$$(n_i)[log(\alpha 1_{i = 0}) + log(\beta \mu^i e^{-\mu}) + log((1-\alpha-\beta)\lambda^ie^{-\lambda})] + $$
...
B <- 1500
y <- matrix(c(chain[,1],chain[,2],chain[,3],chain[,4]),nrow = N,ncol = 4)
mN <- matrix(0,nrow = B,ncol = 4)
for(k in 1:B){
  samp <- y[sample(1:N,size = N, replace = TRUE),]
  mN[k,] <- c(median(samp[,1]),median(samp[,2]),median(samp[,3]),median(samp[,4]))
}
mB <- mean(mN)
var.inner <- mN - mB
var.boot <- (1/(B-1))*sum(var.inner^2)
var.boot
